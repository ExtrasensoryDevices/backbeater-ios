...
//	CGColorRef knobStartColor = [UIColor colorWithWhite:0.82 alpha:1.0].CGColor;
    CGColorRef kStartColor = self.knobStartColor.CGColor;
	CGColorRef kEndColor = (self.gripped) ? [UIColor colorWithWhite:0.894 alpha:1.0].CGColor : [UIColor colorWithWhite:0.996 alpha:1.0].CGColor;

	CGPoint topPoint = CGPointMake(0, 0);
	CGPoint bottomPoint = CGPointMake(0, knobRadius + 2);
	CGGradientRef knobGradient = CreateGradientRefWithColors(colorSpace, kStartColor, kEndColor);
...
-(UIColor*) knobStartColor {
    if (knobStartColor == nil){
        self.knobStartColor = [UIColor colorWithWhite:0.82 alpha:1.0];
    }
    return knobStartColor;
}

- (void)dealloc
{
    [knobStartColor release];
    [super dealloc];
}
